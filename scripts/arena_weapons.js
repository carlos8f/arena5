...
(function()
{
   ...
   extend(Arena.Bullet, Game.Actor,
   {
      ...
      onRender: function onRender(ctx, world)
      {
         ...
         if (this.worldToScreen(ctx, world, this.BULLET_HEIGHT) &&
             this.lifespan < this.BULLET_LIFESPAN - 1)   // hack - to stop draw over player ship
         {
            ...
            // rotate the bullet diamonds into the correct heading
            ctx.rotate(this.heading * RAD);
            
            // draw bullet primary weapon
            ctx.beginPath();
            ctx.moveTo(-this.BULLET_WIDTH*2 , 0);
            ctx.lineTo(-this.BULLET_WIDTH, -this.BULLET_HEIGHT*0.5);
            ctx.lineTo(this.BULLET_WIDTH, 0)
            ctx.lineTo(-this.BULLET_WIDTH, this.BULLET_HEIGHT);
            ctx.moveTo(-this.BULLET_WIDTH , 0);
            ctx.lineTo(this.BULLET_WIDTH, -this.BULLET_HEIGHT*0.5);
            ctx.lineTo(this.BULLET_WIDTH*2, 0)
            ctx.lineTo(this.BULLET_WIDTH, this.BULLET_HEIGHT);
            ctx.closePath();
            ctx.fill();
         }
